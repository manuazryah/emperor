...
<table class="main-tabl" border="0" id="pdf">
    ...
    <tbody>
        ...
        <tr>
            <td>
                <div class="heading" style="margin-bottom: 8px;">TAX INVOICE</div>
                <div class="heading" style="margin-bottom: 10px;font-size: 13px;"><?= OnAccountOf::findOne($invoice->on_account_of)->invoice_name; ?></div>
                <div class="heading-top" style="margin-bottom: 100px;">
                    <div class="main-left">
                        <div style="font-size: 11px;">To,</div>
                        <div style="width: 275px;font-size: 9px;padding-left: 22px;padding-top: 5px;">
                            <?= $invoice->to_address; ?>
                            <?php
                            $tax_id = '';
                            $debtor_data = common\models\Debtor::find()->where(['id' => $invoice->invoice])->one();
                            if (!empty($debtor_data)) {
                                if ($debtor_data->tax_id != '') {
                                    $tax_id = $debtor_data->tax_id;
                                }
                            }
                            if ($tax_id != '') {
                                ?>
                                <p style="font-weight: bold;">TRN : <?= $tax_id ?></p>
                            <?php }
                            ?>
                        </div>
<!--                        <table class="tb2">
                            <tr>
                                ...
                                <td style="max-width: 212px;"><?php // $invoice->to_address;             ?></td>
                            </tr>
                        </table>-->
                    </div>
                    <div class="main-right">
                        <table class="tb2">
                            ...
                            <tr style="font-weight:600;">
                                ...
                                <td style="max-width: 200px">100353326000003</td>
                            </tr>
                        </table>
                    </div>
                </div>
                    ...
                    <table class="table">
                        ...
                        foreach ($invoice_details as $value) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php
                                    if ($value->tax_amount != '' && $value->tax_amount > 0) {
                                        $res = ($value->tax_amount / $value->total) * 100;
                        ...
                        <tr>
                            ...
                            <td style="font-size: 10px;font-weight: bold;">
                                <?php
                                if ($invoice->currency == 1) {
                                    echo 'AED ' . Yii::$app->SetValues->NumberFormat($tax_total);
                                } else {

                                    echo 'USD ' . Yii::$app->SetValues->NumberFormat($tax_total);
                                }
                                ?>
                            </td>
                        </tr>
                    </table>
                    <table class="table" style="width: 30%;float: right;margin-top: 10px;font-weight: 700;">
                        <tr>
                            <td style="text-align:left;font-size: 10px;">Sub Total</td>
                            <td style="text-align:right;font-size: 10px;"><?= Yii::$app->SetValues->NumberFormat(round($grand_total, 2)); ?></td>
                        </tr>
                        <tr>
                            <td style="text-align:left;font-size: 10px;">Tax Amount</td>
                            <td style="text-align:right;font-size: 10px;"><?= Yii::$app->SetValues->NumberFormat(round($tax_total, 2)); ?></td>
                        </tr>
                        <tr>
                            <td style="text-align:left;font-size: 10px;">TOTAL</td>
                            <td style="text-align:right;font-size: 10px;"><?= round(($grand_total + $tax_total), 0); ?></td>
                        </tr>
                    </table>
                </div>
            </td>
            ...
            <td>
                <div class="amount-word" style="margin-top: 75px;">
                    <?php if ($invoice->currency == 1) { ?>
                        <p style="font-size: 12px;font-weight: bold;font-style: italic;">AED <?php echo ucwords(Yii::$app->NumToWord->ConvertNumberToWords(round(($grand_total + $tax_total), 0))) . ' Only'; ?> </p>
                    <?php } else {
                        ?>

                        <p style="font-size: 12px;font-weight: bold;font-style: italic;">USD <?php echo ucwords(Yii::$app->NumToWord->ConvertNumberToWords((round(($grand_total + $tax_total), 0)), 'USD')) . ' Only'; ?> </p>
                    <?php }
                    ?>
