            ...
            <div class="panel-body">
                        ...
                        <tbody>
                            ...
                            foreach ($invoice_details as $invoice_detail) {
                                ...
                                <tr>
                                    ...
                                    <td><?= $invoice_detail->tax_amount; ?></td>
                    ...
                    <table cellspacing="0" class="table table-small-font table-bordered table-striped">
                        ...
                        <tbody>
                            <tr class="filter">
                                ...
                                <td>
                                    <?php $taxes = ArrayHelper::map(common\models\TaxMaster::findAll(['status' => 1]), 'id', 'name'); ?>
                                    <?= $form->field($model, 'tax')->dropDownList($taxes, ['prompt' => '-Choose a Tax-'])->label(false) ?>
                                </td>
                                <td><?= $form->field($model, 'tax_amount')->textInput(['placeholder' => 'Tax Amount', 'readonly' => TRUE])->label(false) ?></td>
                ...
                <script>
                    $(document).ready(function () {
                        ...
                        $("#invoicegeneratedetails-tax").change(function () {
                            var tax = $(this).val();
                            var total = $("#invoicegeneratedetails-total").val();
                            $.ajax({
                                url: '<?= Yii::$app->homeUrl; ?>invoice/generate-invoice/calculate-tax',
                                type: "POST",
                                data: {tax: tax, total: total},
                                success: function (data) {
                                    var res = $.parseJSON(data);
//                                    $("#invoicegeneratedetails-total").val(res.result['total-amount']);
                                    $("#invoicegeneratedetails-tax_amount").val(res.result['tax-amount']);
                                }
                            });
                        });
                    });
