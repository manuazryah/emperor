...
class AppointmentController extends Controller {
    ...
    public function actionCreate() {
        ...
        if ($model->load(Yii::$app->request->post()) && Yii::$app->SetValues->Attributes($model) && $this->Principal($model, $_POST['Appointment']['principal'])) {
            ...
            if ($model->save()) {
                ...
                if (!empty($user)) {
                    $this->SendEmail($model, $user);
                }
                $super_users = \common\models\Employee::find()->where(['status' => 1])->andWhere(['<>', 'id', $model->CB])->all();
    ...
    public function SendEmail($model, $user_info) {
        $to = $user_info->email;
// subject
        $subject = ' Attention : ' . $user_info->name . ' ( New Appointment Created )';

        $message = '<html><head></head><body>';
        $message .= '<p>New Appointment <span style="color: #2196F3;">' . $model->appointment_no . '</span> created on <span style="color: #2196F3;">' . $model->DOC . ' </span></p>';
        $message .= '</body></html>';
