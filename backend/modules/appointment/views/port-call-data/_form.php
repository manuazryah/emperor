...
<div class="port-call-data-form form-inline">
        ...
        if (!empty($model_additional)) {
            ...
            foreach ($model_additional as $data) {
                ...
                <span>
                    ...
                    <div class="form-group">
                        <a id="remScnt" val="<?= $data->id; ?>" class="btn btn-icon btn-red remScnts" ><i class="fa-remove"></i></a>
                    </div>
                </span>
                <br>
                <?php
    ...
    <div class="row">
        <div class="col-md-12">
            <a id="addComments" title="Add More Comments" class="btn btn-icon btn-blue addComments" style="float:right;margin-right: 28px;"><i class="fa-plus"></i></a>
        </div>
    </div>
    <hr class="appoint_history" />
    <?php
...
<script>
    $(document).ready(function () {
        $(document).on('change', '.port-department', function (e) {
            var row_id = $(this).attr('id').match(/\d+/);
            var dep = $(this).val();
            $.ajax({
                url: '<?= Yii::$app->homeUrl; ?>appointment/port-call-data/department-change',
                type: "POST",
                data: {department: dep},
                success: function (data) {
                    $('#port_assign_to-' + row_id).html(data);
                }
            });
        });
        /*
        ...
        $('#p_scents').on('click', '.remScnts', function () {
            $(this).parents('span').remove();
            if (this.hasAttribute("val")) {
                var valu = $(this).attr('val');
                $('#delete_port_vals').val($('#delete_port_vals').val() + valu + ',');
